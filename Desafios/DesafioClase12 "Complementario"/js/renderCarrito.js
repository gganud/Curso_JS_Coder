...
const mostrarCarrito = () =>{    
    ...
    ordenSinRepetidos.forEach(i => {
        ...
        eliminar = document.getElementById(`eliminar${id}`);
        eliminar.addEventListener('click', () => eliminarArticulo(i));
        sumar = document.getElementById(`sumar${id}`);
        sumar.addEventListener('click', () => sumarArticulo(i)); 
        restar = document.getElementById(`restar${id}`);
...
const totalPedido = (lista) => {
    ...
    boton.innerHTML = `<a href="./pages/compra.html" ><button class="button">Ordene su pedido</button></a>`
